...
def run_till_loss(exp_cmd, mode, min_rate, pkt_size, duration, pktgen):
    curr_t = min_rate
    prev_row=[]
    while True:
...
def run_all(experiments, min_rate, pkt_size, duration, pktgen):
    for x in experiments:
        write_row_to_files(x)

        # # throughput
        # row = run_till_loss(x, 0, min_rate, pkt_size, duration, pktgen)
        # write_row_to_file(TP_FILENAME, row)

        # cache
        row = run_till_loss(x, 1, min_rate, pkt_size, duration, pktgen)
        write_row_to_file(CACHE_FILENAME, row)


        # # latency
        # row = run_till_loss(x, 2, min_rate, pkt_size, duration, pktgen)
