...
def run_once(exp_cmd, mode, curr_t, pkt_size, duration, pktgen):
    ...
    while True:
        ...
        pktgen.sendline(f'run_burst("{duration}","{INTERVAL}","{RATE}","{BURST_COUNT}")')
...
def run_all(experiments, max_rate, pkt_size, duration, pktgen):
    for x in experiments:
        ...
        row = run_once(x, 1, max_rate, pkt_size, duration, pktgen)
...
app_type = [3]
bursts=[1024,2048,4096,8192,16384]
rx_queue =[2048]
INTERVAL = 1
...
for b in bursts:
    ...
    write_row_to_files(["Burst count", str(b)])
    for i in range(len(app_type)):
        experiments = [
           ['sudo', APP_PATH, '-l' ,'0' ,'-n', '2', '--' ,'-q' ,'1', '-p', '1', '-A', str(app_type[i]),'-R',str(2048)],
            ['sudo', APP_PATH, '-l' ,'0' ,'-n', '2', '--' ,'-q' ,'1', '-p', '1', '-A', str(app_type[i]),'-R',str(2048),'-S']
        ]
