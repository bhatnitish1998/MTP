...
CACHE_FILENAME = 'cold_ideal_count_123.csv'
...
def run_once(exp_cmd, curr_t, pkt_size, duration, pktgen):
    ...
    print(f'Target {curr_t} Gbps, loss {loss * 100:.2f}%, warm {warm_count} , cold {cold_count} \n')
...
def run_all(experiments, max_rate, pkt_size, duration, pktgen):
    for x in experiments:
        row = run_once(x, max_rate, pkt_size, duration, pktgen)
...
for i in range(len(app_type)):
    ...
    write_row_to_file(myrow)

    experiments = [
    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U','16384','-s','512','-B','-A',str(app_type[i]),'-W'],

    ]
    run_all(experiments, ideal_rate[i], 512, 30000, pktgen)
