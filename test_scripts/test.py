...
experiments = [
    ...
    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U', '16384','-h'],

    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U', '4096'],
    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U', '4096','-u'],

    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U', '16384','-f','2048'],
    ['taskset', '-c', '0', 'sudo', APP_PATH, '-i', IFNAME, '-U', '16384','-b','128'],
...
def run_till_zero(exp_cmd, mode, pattern):
    ...
    while True:
        rx_queue_full, row = run_once(exp_cmd, mode, pattern, curr_t)
        if rx_queue_full == 0:
...
change_ddio(0)
write_row_to_files(["DDIO OFF"])
change_prefetch(0)
write_row_to_files(["PF OFF"])
run_all()
change_prefetch(1)
write_row_to_files(["PF ON"])
run_all()
